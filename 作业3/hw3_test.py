...
"""# Dataset
在 Pytorch 中，我們可以利用 torch.utils.data 的 Dataset 及 DataLoader 
來"包裝" data，使後續的 training 及 testing 更為方便。

Dataset 需要 overload 兩個函數：\_\_len\_\_ 及 \_\_getitem\_\_

\_\_len\_\_ 必須要回傳 dataset 的大小，而 \_\_getitem\_\_ 
則定義了當程式利用 [ ] 取值時，dataset 應該要怎麼回傳資料。

實際上我們並不會直接使用到這兩個函數，但是使用 DataLoader 在 
enumerate Dataset 時會使用到，沒有實做的話會在程式運行階段出現 error。
"""
...
train_transform = transforms.Compose([
    ...
    transforms.Normalize([0.34361264, 0.45097706, 0.5550434], [0.2783836, 0.27154413, 0.26915196])  # 归一化
])
# testing 時不需做 data augmentation
test_transform = transforms.Compose([
    transforms.ToPILImage(),
    transforms.ToTensor(),  # 将图片转成 Tensor
    transforms.Normalize([0.35057753, 0.45622048, 0.5620281], [0.2769976, 0.27097332, 0.26702392])  # 归一化
])
